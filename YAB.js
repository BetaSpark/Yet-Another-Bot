...
(function() {
    ...
    settings.prototype.songIntervalMessages = [
      {
        interval: 7,
        offset: 0,
        msg: "Hit that woot button"
      }, {
        interval: 5,
        offset: 0,
        msg: "We do not like AFK People in the room, Please talk in the chat every now and again!"
      }
    ];
  ...
  pupOnline = function() {
    ...
    myname = me.username;
    currentversion = "1.0";
    log("SuperBOT " + currentversion + " started");
    return API.sendChat("Hey, " + myname + " Is Here!");
  };
      ...
      if (timeSinceLastActivity > data.afkTime) {
        if (user.getIsDj()) {
          ...
          if (user.getWarningCount() === 0) {
            user.warn();
            _results.push(API.sendChat("@" + user.getUser().username + ", I Have not seen you speak for over an hour, if you do not show activity i will remove you."));
            ...
            if (timeSinceLastWarning > twoMinutes) {
              ...
              if (DJs.length > 0 && DJs[0].id !== user.getUser().id) {
                API.sendChat("@" + user.getUser().username + ", I Gave you 1 warning, Please stay active in chat");
